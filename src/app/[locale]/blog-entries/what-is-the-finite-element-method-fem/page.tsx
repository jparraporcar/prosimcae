...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: t(blog.blogEntries[0].canonical),
    },
  };
}
...
const WhatIsTheFiniteElementMethodFem = () => {
  ...
  return (
    <div className="flex flex-row justify-center w-full">
      <div className="w-[75%] max-md:w-[97%] px-4 py-8 sm:px-6 lg:px-8 mb-8 mt-16">
        <Card className="bg-white shadow-2xl rounded-2xl overflow-hidden">
          <CardHeader className="p-6 bg-slate-300">
            <div className="flex justify-between items-center mb-4">
              <Badge
                className={`${getDifficultyColor(t(blog.blogEntries[0].difficulty) as DifficultyLevel)} text-xs max-md:text-[10px] font-semibold px-3 py-1`}
              >
            ...
            <div>
              <article className="prose prose-lg max-w-none">
                <h2 className="text-2xl max-md:text-base font-semibold text-gray-800 mt-8 mb-2">
                  {t(blog.blogEntries[0].content.heading1)}
                </h2>
                <div
                  className="mb-6 text-gray-700 max-md:text-xs"
                  dangerouslySetInnerHTML={{
                    __html: t.raw(blog.blogEntries[0].content.paragraph1),
                  }}
                />
                <h2 className="text-2xl max-md:text-base font-semibold text-gray-800 mt-8 mb-4">
                  {t(blog.blogEntries[0].content.heading2)}
                </h2>
                <div
                  className="mb-6 text-gray-700 max-md:text-xs"
                  dangerouslySetInnerHTML={{
                    __html: t.raw(blog.blogEntries[0].content.paragraph2),
                  }}
                />
                <h2 className="text-2xl max-md:text-base font-semibold text-gray-800 mt-8 mb-4">
                  {t(blog.blogEntries[0].content.heading3)}
                </h2>
                <div
                  className="mb-6 text-gray-700 max-md:text-xs"
                  dangerouslySetInnerHTML={{
                    __html: t.raw(blog.blogEntries[0].content.paragraph3),
                  }}
                />
                <h2 className="text-2xl max-md:text-base font-semibold text-gray-800 mt-8 mb-4">
                  {t(blog.blogEntries[0].content.heading4)}
                </h2>
                <div
                  className="mb-6 text-gray-700 max-md:text-xs"
                  dangerouslySetInnerHTML={{
                    __html: t.raw(blog.blogEntries[0].content.paragraph4),
                  }}
                />
              </article>
            </div>
...
export default WhatIsTheFiniteElementMethodFem;
